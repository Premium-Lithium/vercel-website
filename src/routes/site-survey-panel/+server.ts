...
async function createInspectionFrom(PLNumber: string, templateName: string) {
	try {
		...
		const response = await surveyDataSource.startSurveyFor(
			PLNumber,
			personName,
			propertyAddress,
			templateName
		...
		if (response.ok) {
			...
			return json({
				message: 'Error starting inspection with deal data',
...
async function getStatusFromInspection(PLNumber: string, templateName: string) {
	try {
		...
		let statusResponse;
		if (status) {
			if (status === 'Completed') statusResponse = await crm.setSurveyStatusFor(PLNumber, 'Yes');
...
async function attachPDFToDeal(PLNumber: string) {
	try {
		...
		return json({
			message: 'PDF succesfully attached to deal.',
			statusCode: 200,
			pdfLink: responseData.url,
...
async function updatePipedriveDealFrom(PLNumber: string) {
	try {
		...
		const answerObject = await surveyDataSource.fetchAnswersFromFields(
			PLNumber,
			fieldNames,
			templateName
		);
		...
		else return json({ message: 'Failed to update custom fields.', statusCode: 500 });
	} catch (error) {
