<script lang="ts">
    ...
    */
	...
	let tariffButtonOptions = [
		// each is [<string>, <tis tariff>]
		'Fixed',
		'Variable',
		'Off-Peak',
		'Capped',
...
<!--  Add extra final div for savings - hidden until all other data is input -->

<div>
	<p>What tariff type are you paying for you electricity?</p>
	{#each tariffButtonOptions as option, i}
		<button
			class:selected={tariffType === option}
			class="tariff-button"
			on:click={() => tariffSelected(option)}>{option}</button
		>
	{/each}
	<br />
		...
		<form>
			...
			<label
				>Night start<input
					...
					max="24"
					step="1"
					bind:value={offPeakStart}
				/></label
			>{offPeakStart}<br />
			<label
				>Night end<input type="range" min="0" max="24" step="1" bind:value={offPeakEnd} /></label
			>{offPeakEnd}<br />
		</form>
	...
	{:else}
        ...
        <!-- Todo make this button slide to the next page -->
		<br /><button on:click={() => savingStage++}>Go!</button>
	{/if}
</div>
...
<div>
	...
	<div class="energyDiv">
		...
		<div>
			<button on:click={() => (customizeDayEnergyVisible = !customizeDayEnergyVisible)}
				>Customize></button
			>
			{#if customizeDayEnergyVisible}
				<div class="customise-energy">Customisable bar chart goes here?</div>
			{/if}
		</div>
	</div>
	...
	<div class="energyDiv">
		...
		<div>
			<button on:click={() => (customizeMonthEnergyVisible = !customizeMonthEnergyVisible)}
				>Customize></button
			>
