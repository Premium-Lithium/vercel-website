	...
	<input
		...
		on:click={async () => {
			...
			res.outputs.monthly.fixed.forEach((x) => {
				$allQueryParameters.monthlySolarGenerationValues = [...monthlySolarGenerationValues, x.E_m];
			});
		}}
	/>
</div>
...
<div>
	{#if !loadingSolarValues}
		...
		<SolarGenerationBreakdown bind:monthlyValues={$allQueryParameters.monthlySolarGenerationValues} />
	{/if}
</div>
