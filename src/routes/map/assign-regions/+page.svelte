<script>
	...
	import {
		...
		fetchInstallerDataFromPipedrive,
		fetchJobDataFromPipedrive,
	...
	var swatchColours = ['green', 'blue', 'orange', 'maroon', 'cyan', 'red', 'pink', 'yellow']
	...
	onMount(async () => {
		...
		map.on('load', async () => {
			...
			for (let postcode in data) {
				...
				const popup = new mapboxgl.Popup({ offset: 25 }).setText(
					pointInPolygonFromList(
						point([data[postcode].longitude, data[postcode].latitude]),
						polygons
					)
				)
		...
		const updatePolygonCoordinates = (e) => {
			...
			let pointMarkerList = markerIdList.map((m) => {
				return { 'marker': m.marker, 'point': point(m.marker.getLngLat().toArray()) }
			})
			...
			polygons.forEach((currentElement) => {
				if (e && e.features && e.features.length === 1) {
					...
					draw.setFeatureProperty(
						e.features[0].id,
						'myFillColorProperty',
						swatchColours[polyColour]
					)
				}
				const poly = currentElement
				const pointsInside = pointsWithinPolygon(points, poly).features
				pointsInside.forEach((point) => {
					let p = pointMarkerList.find(
						(obj) =>
							obj.point.geometry.coordinates[0] === point.geometry.coordinates[0] &&
							obj.point.geometry.coordinates[1] === point.geometry.coordinates[1]
					).marker
				})
			})
		...
		async function saveRegionsToDB() {
			for (let i = 0; i < polygons.length; i++) {
				await supabase.from(DB_NAME).upsert({
		...
		async function syncJobOwnersToPipedrive() {
			...
			let res = await fetch(
				`https://api.pipedrive.com/api/v1/deals/${dealId}?api_token=77a5356773f422eb97c617fd7c37ee526da11851`,
				{
					method: 'PUT',
...
{#if !isAuthenticated}
	<MagicLink redirectUrl="map/assign-regions" />
{:else}
